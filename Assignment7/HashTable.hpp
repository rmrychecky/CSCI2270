...
class HashTable {
  public:
    ...
    ~HashTable();
    void addWord(std::string word); //okay
    bool isInTable(std::string word);
    void incrementCount(std::string word);
    void printTopN(int n); //okay
    int getNumCollisions();
    int getNumItems();
    int getTotalNumWords(); //okay
...
void getStopWords(char *ignoreWordFileName, HashTable &stopWordsTable); //okay
