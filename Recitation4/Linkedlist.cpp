...
bool LinkedList::deleteAtIndex(int n)
{
  ...
  for (int i = 0; pres!=NULL && i < n -1; i++){ //finding where it is
    pres=pres->next;
  }
  
  if(pres == NULL || pres->next == NULL){ //if the given position is more than the number of nodes
    isDeleted = false;
    return isDeleted;
  }
  
  Node *next = pres->next->next; //making a temp next node
